...
- [ ] T001 创建工程声明式配置文件 `WalkMate/project.yml`，定义 Bundle ID (`accera.insta.dap`)、编译目标（iOS 17.0+）、框架依赖与签名配置
...
- [ ] T015 [P] [US1] 实现六轴传感器遥测数据 HUD 视图 `WalkMate/App/Views/SensorTelemetryCard.swift`（包含 VoiceOver 语义支持）
- [ ] T016 [US1] 实现主控制界面 `WalkMate/App/ContentView.swift`（整合连接/断开控制按钮、全景画面与传感器卡片，确保触控尺寸 >= 48x48pt）
...
- [ ] T018 [P] [US2] 实现重力对齐器 `WalkMate/Core/Geometry/GravityAligner.swift`（根据 IMU 姿态四元数旋转点云，使 Z 轴垂直于重力方向）
- [ ] T019 [US2] 实现纯动态地面估计器 `WalkMate/Core/Geometry/GroundPlaneEstimator.swift`（下半球点云 RANSAC 快速平面拟合，动态计算离地高度与地面剥离）
- [ ] T020 [US2] 实现 BEV 栅格与通行走廊规划器 `WalkMate/Core/Analysis/PassageCorridorPlanner.swift`（300x300 鸟瞰栅格构建 + EDT 欧氏距离变换，按 0.6 米人体宽度提取中心中轴线与 `targetAnchor`）
...
- [ ] T030 执行 `xcodegen generate` 生成 Xcode 原生工程并验证编译通过
...
Phase 2: Foundational (T004 ~ T010) [阻塞后续所有用户故事]
  ↓
Phase 3: User Story 1 (T011 ~ T016) [MVP 核心硬件数据源]
  ↓
Phase 4: User Story 2 (T017 ~ T021) [空间通道导向]
  ↓
Phase 5: User Story 3 (T022 ~ T024) [前向避障]
  ↓
Phase 6: User Story 4 (T025 ~ T026) [防踩空] & Phase 7: User Story 5 (T027 ~ T029) [后退雷达]
  ↓
